        ...
        for py_file in adapters_path.rglob("*.py"):
            ...
            with open(py_file, 'r') as f:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.Import):
                        for alias in node.names:
                            if (alias.name.startswith("src.app") and
                                not alias.name.startswith("src.app.domain") and 
                                not alias.name.startswith("src.app.infra") and
                                not alias.name.startswith("src.app.adapters")):
                                forbidden_imports.append(f"{py_file}: {alias.name}")
                    elif isinstance(node, ast.ImportFrom):
                        if (node.module and node.module.startswith("src.app") and
                            not node.module.startswith("src.app.domain") and 
                            not node.module.startswith("src.app.infra") and
                            not node.module.startswith("src.app.adapters")):
                            forbidden_imports.append(f"{py_file}: from {node.module}")
        
        assert not forbidden_imports, f"Adapters import forbidden modules: {forbidden_imports}"
                    ...
                    if isinstance(node, ast.FunctionDef) and node.name == "__init__":
                        for arg in node.args.args:
                            if arg.annotation:
                                ...
                                if ("Manager" in annotation_str and 
                                    "Protocol" not in annotation_str and
                                    "Port" not in annotation_str):
                                    concrete_type_usage.append(f"{py_file}: {arg.arg} - {annotation_str}")
        
        assert not concrete_type_usage, f"Adapters using concrete types: {concrete_type_usage}"
                        ...
                        for arg in node.args.args:
                            if arg.annotation:
                                ...
                                if "Settings" in annotation_str:
                                    settings_usage.append(f"{py_file}: {arg.arg} - {annotation_str}")
